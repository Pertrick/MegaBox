...
class PaymentAction{
    ...
    public function paymentCheckout(Request $request){
        ...
        if($response && $response['status']== true) {
          ...
          DB::transaction(function () use ($user, $email, $uploadedData, $reference): void {

              $payment = new Payment();
              $payment->savePayment($user, $email, 'data', $reference, 'NGN', 100);

              $data = new Data();
              $data->saveData($uploadedData,$email);
